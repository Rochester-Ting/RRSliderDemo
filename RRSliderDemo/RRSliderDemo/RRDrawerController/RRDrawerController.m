...
typedef NS_ENUM(NSInteger, RRDrawerControllerType) {
    RRDrawerControllerTypeNone,
    RRDrawerControllerTypeCloseLeft,
    RRDrawerControllerTypeOpenLeft,
    RRDrawerControllerTypeOpenRight,
    RRDrawerControllerTypeCloseRight
};

@interface RRDrawerController ()<UIGestureRecognizerDelegate, UINavigationControllerDelegate> {
...
@property (assign, nonatomic) RRDrawerControllerType operationType;
...
@implementation RRDrawerController
...
- (void)commonInit {
    ...
    _drawerControllerStyle = RRDrawerControllerStyleParallaxSlide;
    _drawerControllerOpenStyle = RRDrawerControllerOpenStyleFromAnyWhere;
    self.isAnimating = NO;
...
//    }
    ...
    // 添加手势到centerContentView上面, 因为我们希望只有内容的view上面能够响应手势
    // 手势的初始化我们使用了懒加载
...
- (void)openLeftDrawerAnimated:(BOOL)animated finishHandler:(FinishHandler)finishHandler {
    if (_leftController) {
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            if (self.drawerControllerStyle == RRDrawerControllerStyleScale) {
    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateBegan:
            ...
            if (self.isAnimating) {// 正在动画就停止pan手势
                ...
                /// 主控制器的frame
                /// 获取centerContentView的当前位置
                _beginCenterContentViewFrame = self.centerContentView.frame;
                /// 获取drawerContentView的当前位置
                _beginDrawerContentViewFrame = self.drawerContentView.frame;
                if (_beginCenterContentViewFrame.origin.x == 0) { // centerContentView在中间位置
                    _beginningScale = 1.0;
                    if (velocityX < 0 && _rightController) { // x<0说明向左或者向上滑动且rightVC出现了那么打开右边
                        self.operationType = RRDrawerControllerTypeOpenRight;
                    }
                    else if  (velocityX > 0 && _leftController) { // x<0说明向下或者向右滑动且leftVC出现了那么打开左边
                        self.operationType = RRDrawerControllerTypeOpenLeft;
                    }
                    else {
                        
                        self.operationType = RRDrawerControllerTypeNone;
                    }
                }
                else {
                    _beginningScale = _minimumScale;
                    if (self.centerContentView.center.x > self.view.center.x && velocityX < 0 && _leftController) { //如果centerContentView的中心点在self.view的中心点的右边且向左滑动且leftVC存在,那么关闭左边
                        self.operationType = RRDrawerControllerTypeCloseLeft;
                    }
                    else if (self.centerContentView.center.x < self.view.center.x && velocityX > 0 && _rightController) {
                        // 如果centerContentView的中心点在self.view的左边,向右滑动,rightVC存在,关闭右边
                        self.operationType = RRDrawerControllerTypeCloseRight;
                    }
                    else {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            if (transtion.x < 0) {//左滑
                ...
                if (self.drawerControllerStyle == RRDrawerControllerStyleScale) {
                    ...
                    if (self.operationType == RRDrawerControllerTypeOpenLeft && _leftController) {
                    ...
                    if (self.operationType == RRDrawerControllerTypeCloseLeft && _leftController) {  // 关闭左边抽屉
                        [self hideRightShowLeft];
                    ...
                    if(self.operationType == RRDrawerControllerTypeOpenRight && _rightController) {// 打开右边抽屉
                        [self hideLeftShowRight];
            ...
            if (transtion.x > 0) { // 右滑
                ...
                if (self.drawerControllerStyle == RRDrawerControllerStyleScale) {
                    ...
                    if (self.operationType == RRDrawerControllerTypeOpenRight) {
                    ...
                    if (self.operationType == RRDrawerControllerTypeOpenLeft && _leftController) { // 打开左边抽屉
                        [self hideRightShowLeft];
                        CGFloat progress = transtion.x/_maxLeftControllerWidth;
                        scale = scale - (1-_minimumScale)*progress;
                    ...
                    if(self.operationType == RRDrawerControllerTypeCloseRight && _rightController) {// 关闭右边抽屉
                        [self hideLeftShowRight];
        ...
        case UIGestureRecognizerStateCancelled: {
            self.isAnimating = NO;
            if (self.operationType == RRDrawerControllerTypeNone) break;
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (self.drawerControllerOpenStyle == RRDrawerControllerOpenStyleFromScreenEdge) {
        ...
        if (self.delegate && [_delegate respondsToSelector:@selector(RR_drawerController:shouldBeginPanGesture:)]) {
            return [self.delegate RR_drawerController:self shouldBeginPanGesture:self.panGesture];
        }
...
- (void)setDrawerControllerStyle:(RRDrawerControllerStyle)drawerControllerStyle {
    _drawerControllerStyle = drawerControllerStyle;
    switch (drawerControllerStyle) {
        case RRDrawerControllerStyleParallaxSlide:
            kPalaxPercent = 0.7f;
            break;
        case RRDrawerControllerStyleNone:
            kPalaxPercent = 0.0f;
            break;
        case RRDrawerControllerStyleScale:
            kPalaxPercent = 0.0f;
            break;
        default:
            kPalaxPercent = 1.0f;
...
@implementation UIViewController (RRDrawerController)


- (RRDrawerController *)RR_drawerController {
    UIViewController *drawerController = self;
    while (drawerController) {
        if ([drawerController isKindOfClass:[RRDrawerController class]]) {
            break;
        }
        drawerController = drawerController.parentViewController;
    }
    
    return (RRDrawerController *)drawerController;
}
