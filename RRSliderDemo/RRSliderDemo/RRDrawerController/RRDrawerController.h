...
@class RRDrawerController;
typedef NS_ENUM(NSInteger, RRDrawerControllerStyle) {
    RRDrawerControllerStyleNone, //抽屉菜单不滚动
    RRDrawerControllerStyleNormalSlide, // 抽屉菜单hl和中间的controller同步滚动
    RRDrawerControllerStyleParallaxSlide, // 抽屉菜单和中间的controller同时滚动, 但不同步
    RRDrawerControllerStyleScale // 中间的controller滚动时缩放
};

typedef NS_ENUM(NSInteger, RRDrawerControllerOpenStyle) {
    RRDrawerControllerOpenStyleFromScreenEdge, // 从屏幕边缘才能触发
    RRDrawerControllerOpenStyleFromAnyWhere // 任何地方都可以触发
};

@protocol RRDrawerControllerDelegate <NSObject>

- (BOOL)RR_drawerController:(RRDrawerController *)drawerController shouldBeginPanGesture: (UIPanGestureRecognizer *)panGesture;

@end


@interface RRDrawerController : UIViewController
typedef void(^FinishHandler)(BOOL finished);

@property (weak, nonatomic) id<RRDrawerControllerDelegate> delegate;


/** 抽屉的动画样式 默认为 RRSlideControllerStyleParalaxSlide*/
@property (assign, nonatomic) RRDrawerControllerStyle drawerControllerStyle;
/** 打开抽屉手势触发的方式  --- 从屏幕边缘还是全屏 默认为RRSlideControllerOpenStyleFromAnyWhere*/
@property (assign, nonatomic) RRDrawerControllerOpenStyle drawerControllerOpenStyle;
...
/** 滑动手势能够触发的区域宽度 当slideControllerOpenStyle设置为RRSlideControllerOpenStyleFromScreenEdge的时候才生效 默认为80.0f */
...
@interface UIViewController (RRDrawerController)

/** 抽屉菜单 -- 控制器 */
@property (weak, readonly, nonatomic) RRDrawerController *RR_drawerController;
